...
- the table contains all executed commands. The columns 'success?' and 'reason' are populate by asking the LLM if the executed command (and its output) help with getting root access as well as to reason about the commands output
