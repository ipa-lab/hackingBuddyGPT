...
RootedCONRootedCON## Linux Capabilities
...
2. Purpose: Determines the capabilities passed down from the parent process.

3. Functionality: When a new process is created, it inherits the capabilities from its parent in this set. Useful for maintaining certain privileges across process spawns.

4. Restrictions: A process cannot gain capabilities that its parent did not possess.
...
6. Purpose: Represents the actual capabilities a process is utilizing at any moment.

7. Functionality: It's the set of capabilities checked by the kernel to grant permission for various operations. For files, this set can be a flag indicating if the file's permitted capabilities are to be considered effective.

8. Significance: The effective set is crucial for immediate privilege checks, acting as the active set of capabilities a process can use.
...
10. Purpose: Defines the maximum set of capabilities a process can possess.

11. Functionality: A process can elevate a capability from the permitted set to its effective set, giving it the ability to use that capability. It can also drop capabilities from its permitted set.

12. Boundary: It acts as an upper limit for the capabilities a process can have, ensuring a process doesn't exceed its predefined privilege scope.
...
14. Purpose: Puts a ceiling on the capabilities a process can ever acquire during its lifecycle.

15. Functionality: Even if a process has a certain capability in its inheritable or permitted set, it cannot acquire that capability unless it's also in the bounding set.

16. Use-case: This set is particularly useful for restricting a process's privilege escalation potential, adding an extra layer of security.
...
18. Purpose: Allows certain capabilities to be maintained across an `execve` system call, which typically would result in a full reset of the process's capabilities.

19. Functionality: Ensures that non-SUID programs that don't have associated file capabilities can retain certain privileges.

20. Restrictions: Capabilities in this set are subject to the constraints of the inheritable and permitted sets, ensuring they don't exceed the process's allowed privileges.
...
Base on on , and a few files new to be configured to give a user certain capabilities but the one assigning the capabilities to each user will be `/etc/security/capability.conf`.
File example:
...
RootedCONRootedCON## Privesc/Container Escape
...
2. Find a shellcode for the architecture ()

3. Find a program to inject the shellcode into a process memory ()
...
You can learn how the following exploiting works in  but in resume CAP_DAC_READ_SEARCH not only allows us to traverse the file system without permission checks, but also explicitly removes any checks to open_by_handle_at(2) and could allow our process to sensitive files opened by other processes.

The original exploit that abuse this permissions to read files from the host can be found here: , the following is a modified version that allows you to indicate the file you want to read as first argument and dump it in a file.
...
The code of this technique was copied from the laboratory of "Abusing DAC_READ_SEARCH Capability" from 

​

​​​​​​​​​​​ is the most relevant cybersecurity event in Spain and one of the most important in Europe. With the mission of promoting technical knowledge, this congress is a boiling meeting point for technology and cybersecurity professionals in every discipline.

RootedCONRootedCON## CAP_DAC_OVERRIDE

This mean that you can bypass write permission checks on any file, so you can write any file.

There are a lot of files you can overwrite to escalate privileges, .
...
First of all read the previous section that  of the host and compile the exploit.
...
The code of this technique was copied from the laboratory of "Abusing DAC_OVERRIDE Capability" from 
...
If docker is installed you could impersonate the docker group and abuse it to communicate with the .
...
Once you have  you can go to its section to see how to escalate privileges.
...
: Permitted: This is a limiting superset for the effective capabilities that the thread may assume. It is also a limiting superset for the capabilities that may be added to the inheri‐table set by a thread that does not have the CAP_SETPCAP capability in its effective set.
...
RootedCONRootedCON## CAP_NET_BIND_SERVICE
...
The following example is `python2` code that can be useful to intercept traffic of the "lo" (localhost) interface. The code is from the lab "The Basics: CAP-NET_BIND + NET_RAW" from 
...
## CAP_MKNOD
 ...
 extends the functionality of the `mknod` system call beyond creating regular files, FIFOs (named pipes), or UNIX domain sockets. It specifically allows for the creation of special files, which include:
...
It is a default docker capability ().
...
2. Determine your current user ID with `id`, e.g., `uid=1000(standarduser)`.

3. Identify the target device, for example, `/dev/sdb`.

4. Inside the Container as `root`:
...
Most of these examples were taken from some labs of , so if you want to practice this privesc techniques I recommend these labs.
