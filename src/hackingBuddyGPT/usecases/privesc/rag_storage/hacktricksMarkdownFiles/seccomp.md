...
Seccomp-bpf is supported by Docker to restrict the syscalls from the containers effectively decreasing the surface area. You can find the syscalls blocked by default in  and the default seccomp profile can be found here .
...
If you want for example to forbid a container of executing some syscall like `uname` you could download the default profile from  and just remove the `uname` string from the list.
