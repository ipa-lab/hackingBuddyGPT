...
def is_async_callable(fn: Callable[..., R]) -> bool:
    return inspect.iscoroutinefunction(fn) or (callable(fn) and inspect.iscoroutinefunction(fn.__call__))


@overload
def run_maybe_async(fn: Callable[P, Awaitable[R]], *args: P.args, **kwargs: P.kwargs) -> R: ...
@overload
def run_maybe_async(fn: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R: ...


def run_maybe_async(fn: Callable[P, Any], *args: P.args, **kwargs: P.kwargs) -> Any:
    """
    Call `fn` (sync or async) from non-async code and return its result.

    Assumes there's NO running event loop in this thread.
    If `fn(*args, **kwargs)` returns an awaitable, runs it with asyncio.run().
    """
    result = fn(*args, **kwargs)
    if inspect.isawaitable(result):
        return asyncio.run(cast(Awaitable[R], result))
    return result
...
def configurable(name: str, description: str):
    ...
    def inner(cls: C) -> Configurable[C]:
...
def parameter(
    *,
    ...
    choices: dict[str, type] | None = None,
    default: T = MISSING,
    init: bool = True,
    repr: bool = True,
    hash: bool | None = None,
...
NestedCollection = C | dict[str, "NestedCollection[C]"]
...
def get_at(
    collection: NestedCollection[C], name: list[str], at: int = 0, *, meta: bool = False, no_raise: bool = False
) -> C | None:
    if meta:
        name = name + ["$"]
...
def dfs_flatmap(
    collection: NestedCollection[C], func: Callable[[list[str], C], Any], basename: Optional[list[str]] = None
):
    if basename is None:
        basename = []
    output = []
...
class ParameterDefinition(Generic[C]):
    ...
    def __call__(self, collection: ParsingResults) -> C:
        if self._instance is None:
            ...
            if self.type is bool and type(value) is not bool:
...
class ComplexParameterDefinition(ParameterDefinition[C]):
    ...
    def __call__(self, collection: ParsingResults) -> C:
        ...
        if self._instance is None:
            self._instance = self.type(**{name: param(collection) for name, param in self.parameters.items()})
            if hasattr(self._instance, "init"):
                run_maybe_async(self._instance.init)
        return self._instance
...
class ChoiceParameterDefinition(ParameterDefinition[C]):
    ...
    def __call__(self, collection: ParsingResults) -> C:
        if self._instance is None:
            ...
            self._instance = choice(**{name: parameter(collection) for name, parameter in parameters.items()})
            if hasattr(self._instance, "init"):
                run_maybe_async(self._instance.init)
        return self._instance


def get_inspect_parameters_for_class(
    cls: type, basename: list[str]
) -> dict[str, tuple[inspect.Parameter, list[str], Optional[dataclasses.Field]]]:
...
def get_type_description_default_for_parameter(
    parameter: inspect.Parameter, name: list[str], field: Optional[dataclasses.Field] = None
) -> tuple[Type, Optional[str], Any]:
    ...
    if field is not None:
        ...
        if field.type is not None:
            if not (isinstance(field.type, type) or get_origin(field.type) is Union or type(field.type) is UnionType):
                raise ValueError(
    ...
    if (
        (get_origin(parameter_type) is Union or type(field.type) is UnionType)
        and len(parameter_type.__args__) == 2
        and parameter_type.__args__[1] is NoneType
    ):
...
def try_existing_parameter(
    ...
    default: Any,
    description: str,
    secret_parameter: bool,
) -> Optional[ParameterDefinition]:
    existing_parameter = get_at(
        parameter_collection, name, meta=(typ in (ComplexParameterDefinition, ChoiceParameterDefinition))
    )
    ...
    if existing_parameter.default != default:
        if (
            existing_parameter.default is None
            and isinstance(secret_parameter, no_default)
            or existing_parameter.default is not None
            and not isinstance(secret_parameter, no_default)
        ):
...
def parameter_definitions_for_class(
    cls: type, name: list[str], parameter_collection: ParameterCollection
) -> dict[str, ParameterDefinition]:
    return {
        name: parameter_definition_for(*metadata, parameter_collection=parameter_collection)
        for name, metadata in get_inspect_parameters_for_class(cls, name).items()
    }


def parameter_definition_for(
    ...
    *,
    ...
    if parameter_type in (str, int, float, bool):
        existing_parameter = try_existing_parameter(
            parameter_collection,
            name,
            typ=ParameterDefinition,
        ...
        existing_parameter = try_existing_parameter(
            parameter_collection,
            name,
            typ=ComplexParameterDefinition,
            parameter_type=parameter_type,
            default=default,
            description=description,
            secret_parameter=secret_parameter,
...
class Parseable(Generic[C]):
    ...
    def to_help(self, defaults: list[tuple[str, ParsingResults]], level: int = 0) -> str:
        return "\n".join(
            dfs_flatmap(
                self._parameter_collection,
                lambda _, parameter: parameter.to_help(defaults, level + 1)
                if not isinstance(parameter, ComplexParameterDefinition)
                else None,
            )
        )
...
def to_help_for_command(
    program: str, command: list[str], parseable: Parseable[C], defaults: list[tuple[str, ParsingResults]]
) -> str:
...
def _instantiate(
    program: str, args: list[str], commands: CommandMap[C], command_chain: list[str]
) -> tuple[C, ParsingResults]:
    if command_chain is None:
        command_chain = []
...
def get_environment_variables(
    parsing_results: ParsingResults, parameter_collection: ParameterCollection
) -> tuple[str, ParsingResults]:
...
def get_config_file_variables(
    config_file_path: str, parsing_results: ParsingResults, parameter_collection: ParameterCollection
) -> tuple[str, ParsingResults]:
...
def filter_secret_values(
    parsing_results: ParsingResults, parameter_collection: ParameterCollection, basename: Optional[list[str]] = None
) -> ParsingResults:
    if basename is None:
        basename = []
...
def parse_args(
    ...
    parseable: Parseable[C],
    parse_env_file: bool = True,
